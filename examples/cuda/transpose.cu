...
__global__ void transpose_read_coalesced(float *A, float *AT, int m, int n) {
...
__global__ void transpose_write_coalesced(float *A, float *AT, int m, int n) {
    #define A(i, j)  A [(i) * n + (j)] // m x n
    #define AT(i, j) AT[(i) * m + (j)] // n x m
